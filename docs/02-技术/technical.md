...
# v0.3.1 UI 布局方案（2026-09-16）

- `src/ui/window.py` 使用内容测量得到主窗最低所需高度；旧的 `window_size` 只保留宽度偏好，渲染时高度不得低于指标区、进度条和状态区的合计高度。主窗高度上限提升至 560，十项指标无需压缩到不可读字号。
- `src/ui/settings.py` 使用 760×700 左右的双栏布局，不再创建纵向滚动视口。两栏分别独立排布卡片，底部操作按钮固定在窗口底部。
- 设置窗根据屏幕逻辑尺寸取不超过屏幕边缘留白的实际尺寸；两栏内容区域以统一卡片宽度与行高计算，不依赖用户滚动。
- 布局回归测试检查默认四项、全部十项、长错误提示的 Canvas 边界，以及设置窗内容边界和无滚动区约束。
- Windows 单文件包固定使用 Python 3.12.6（Tcl 8.6.13）构建。构建脚本临时将官方 `init.tcl` 改名为 `init_real.tcl`，写入两行引导脚本，先以 `[file dirname [info script]]` 明确设置 `::tcl_library`，再加载官方脚本；打包结束后恢复构建环境原文件。这样可避免 Tcl_Init 已找到解压目录却尚未设置库变量的问题。
