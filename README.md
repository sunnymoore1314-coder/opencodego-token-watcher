...
> v0.3.1（2026-09-16）：主窗会按指标和提示自动完整展开；设置页改为双栏总览，全部常用选项无需滚动即可查看。`OpenCode用量监测.exe` 可直接双击运行，无需安装 Python，也无需复制源码或 assets。
...
- 主窗会按已选指标和提示自动增高，无需手动缩放才能看全；仍可拖四角调整宽度（范围 180×120 至 600×560）
...
- **关于**：版本、数据目录 + 打开数据目录；设置页为双栏总览，无需滚动
...
右键菜单 → 设置，弹出 Apple 风格双栏设置窗（780×680，全部分区直接可见）：
...
```
py -3.12 -m venv .build-venv3126
.build-venv3126\Scripts\python.exe -m pip install -r requirements-build.txt
powershell -File tools\build.ps1
```

发布构建固定使用 Python 3.12.6（Tcl 8.6.13），避开后续 Tcl 8.6.15 在本机的单文件初始化问题。产物为根目录 `OpenCode用量监测.exe`，`dist/opencodego-token-watcher.exe` 保留同内容英文名副本。使用 `python tools/verify_package.py` 可验证独立目录、隔离配置和无 Python PATH 下运行（会显示测试窗口）。
